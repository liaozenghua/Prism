    ...
    for i in range(mcts_task.branch):
        ...
        while not proposal:
            if mcts_task.use_reflection == 'common':
                ...
                response = mcts_task.get_response(node.y,proposal,node.depth + 1)
                qa=proposal+'\n'+response
                answer= mcts_task.get_answer(node.y,qa,node.depth +1)
        if not proposal:
            continue
        next_steps.append({'qa':qa,'answer':answer})
    return next_steps
...
def greedyPolicy(node: treeNode, mcts_task):
    ...
    for i in range(mcts_task.roll_forward_steps):
        ...
        if not actions:
            break
        response = mcts_task.get_response(strs, actions[0], cur_step)
        qa = actions[0] + '\n' + response
        answer = mcts_task.get_answer(strs, qa, cur_step)

        strs =strs+'\n' + qa

        cur_step += 1
        value = mcts_task.get_step_value(strs,answer)
...
def executeRound(root, mcts_task):
    ...
    if mcts_task.reward_model_type == 'vm':
        ...
        if node.reflection == '<end>':
            ...
            best_V = greedyPolicy(roll_node, mcts_task) if mcts_task.roll_policy == 'greedy' else randomPolicy(roll_node,mcts_task)  ##模拟后续步骤，求后续步骤中最大的V
...
def expand(node: treeNode, mcts_task):
    if not node.reflection:
        reflection = mcts_task.get_simple_reflection(node.y, node.depth + 1)  ##获取模型认为当前状态是否推理完毕
        node.update_reflection(reflection)
        if reflection == '<end>':
            print(reflection)
    if node.reflection == '<end>':
    ...
    for action in actions:
        if action['qa'] not in node.children.keys():
            node.append_children(action['qa'],action['answer'])
            child = node.children[action['qa']]
            value = mcts_task.get_step_value(child.y,child.answer)
