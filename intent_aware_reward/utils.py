...
def get_models_and_tokenizers(model_type_llm=None, 
    ...
    if model_type_llm is not None:
        ...
        if model_type_llm in ['opt-125m', 'opt-350m', 'opt-1.3b', 'opt-2.7b', 'opt-6.7b', 'opt-13b', 'opt-30b', 'opt-66b']:
            ...
            opt_path = f'/home/data/liaozenghua/models/opt-1.3b'
    ...
    if model_type_deberta is not None:
        ...
        if model_type_deberta in ["deberta-base-mnli", "deberta-large-mnli", "deberta-xlarge-mnli", "deberta-v2-xlarge-mnli", "deberta-v2-xxlarge-mnli"]:
            deberta_path = f'/home/data/liaozenghua/models/deberta-large-mnli'
            ...
            deberta_model = AutoModelForSequenceClassification.from_pretrained(deberta_path).to('cuda') if not get_tokenizer_only_deberta else None
...
def generate_text(args, 
                  ...
                  decoding_method, 
                  device):
    input_ids = input_ids.to('cuda').reshape(1, -1) if args.dataset == 'trivia_qa' else input_ids.to('cuda')
...
def compute_likelihood(prompt, 
    ...
    for i in range(len(generation['generation_ids'])): 
        ...
        generation_input = torch.hstack([prompt, generation_ids]).to('cuda')  ##拼接prompt和生成文本的id

        target_ids = generation_input.clone()
        target_ids[:len(prompt)] = -100  ##屏蔽prompt的损失，只计算生成文本的损失
        model_output = model(torch.reshape(generation_input, (1, -1)), labels=target_ids)  ##计算总logits
        average_neg_log_likelihood = model_output['loss'].item()  ##计算生成文本的平均损失
        neg_log_likelihood = average_neg_log_likelihood * (len(generation_ids))  ##计算生成文本的总损失
        ...
        # compute logits
        if store_logits:
            generation_logits = model_output["logits"][0, len(prompt)-1:-1, :].to('cpu')  ##生成文本的logits
        ...
        if compute_cleaned:
            ...
            if torch.equal(cleaned_generation_ids, generation_ids) or \
                ...
                generation_input = torch.hstack([prompt, cleaned_generation_ids]).to('cuda')
                target_ids = generation_input.clone()
            ...
            for k, genration_key in enumerate(list_iterations):
                ...
                if genration_key == 'cleaned_generation_text' and generation_i['generation_text'][0] == generation_i['cleaned_generation_text'][0] \
                    ...
                    if generation_i[genration_key][0].lower() == generation_j[genration_key][0].lower():
                        ...
                        prediction = deberta_model(torch.tensor([encoded_input], device='cuda'))['logits']
    ...
    for k, genration_key in enumerate(list_iterations):
        ...
        for batch_i in num_batches:
            for batch_j in num_batches:
                ...
                try:
                    # try batched input
                    encoded_input = deberta_tokenizer(model_input, return_tensors='pt', padding=True).to('cuda')
