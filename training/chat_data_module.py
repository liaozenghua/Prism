...
#         input_messages.append(messages[0])
        ...
        for role, msg in zip(roles, messages):
            if role == "SYSTEM":
                input_messages.append(system_format.format(content=msg))
            if role == "ASSISTANT":
                input_messages.append(assistant_format.format(content=msg, stop_token=tokenizer.eos_token))
            elif role == "USER":
                input_messages.append(user_format.format(content=msg, stop_token=tokenizer.eos_token))
